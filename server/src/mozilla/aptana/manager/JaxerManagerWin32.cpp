...
static void InitiateSocketPeek(Connection *c)
{
	...
	WSABUF buffers[1];
	buffers[0].len = (c->m_reqType == eNEW_REQUEST_MSG || g_debugCount <= 0) ? 1 : 8192;
                                     ...
                                     LPWSAOVERLAPPED lpOverlapped, DWORD dwFlags)
{
	...
	char* data = c->m_from.m_data;
	// This can be a http request or jaxer-protocol request.
	if (c->m_reqType == eNEW_REQUEST_MSG)
	{
    ...
    }else
	{
		...
		char *jaxer_id = get_jaxer_id_from_http_req(data, cbTransferred);
		if (! jaxer_id)
		{
			StartNewRequest(c);
		}else
		{
			bool bHandled = find_server_and_handle_connection(c, jaxer_id, g_debugServers);
			if ( !bHandled)
			{
				//Cannot find the Jaxer
				Log(eWARN, "  apc_SocketPeekComplete: cannot find the requested debug jaxer %s. Let other Jaxer serving the request %d", jaxer_id, c);
				StartNewRequest(c);
			}
		}
	}
