 ...
 * Jaxer-ID: <jaxer-id>
 * in it.
 * req -- pointing the start of the request body
 * len -- length of req.
 *
 * req will be alterted during the search.
 *
 * If found, return a point pointing to the real id.
 *    The req string is altered.
 *    RETURN only a point, actual data is still in req!!!
 * Return 0 if not found.
 */
char *get_jaxer_id_from_http_req(char* req, int len)
{
	static const char * JAXER_ID="\012Jaxer-ID:";
	static int IDLEN = strlen(JAXER_ID);
	char* jaxer_id = 0;
	req[len] = 0;
	char * JaxerID = strstr(req, JAXER_ID);
	if (JaxerID && *JaxerID)
	{
		// Find the id
		jaxer_id = JaxerID + IDLEN;
		while (jaxer_id && isspace(*jaxer_id)) jaxer_id++;
		char *id_end = jaxer_id;
		while (id_end && !isspace(*id_end)) id_end++;
		*id_end = 0;
	}
	return jaxer_id;
}
...
/*
 * Find the server from the list starting at hdr that match the required jaxer_id,
 * and let it handle the connection c.
 *
 * c -- a new connection
 * jaxer_id -- the jaxer id the connection requires
 * hdr -- a list of servers that may contain the required jaxer
 *
 * return true if the required jaxer is found.  In this case, the request has been handled.
 * return false if the required jaxer cannot be found.  In this case, the request has not been
 *  handled yet!
 */
bool find_server_and_handle_connection(Connection* c, const char* jaxer_id, Server* hdr)
{
	Log(eTRACE, "find_server_and_handle_connection: Look for the required jaxer id=%s for request %d",
		jaxer_id, c);
	...
	while(s)
	{
		...
		{
			//Found the server
			Log(eTRACE, "find_server_and_handle_connection: Required jaxer id=%d found", s->m_pid);
			c->m_boundServer = s;
			c->m_next = NULL;
			// Is the server busy?
			if (s->m_boundConnection)
			{
				Log(eTRACE, "find_server_and_handle_connection: Jaxer id=%d is busy, request %d will be put on waiting list",
					s->m_pid, c);
			...
			{
				Log(eDEBUG, "  find_server_and_handle_connection: Let jaxer %d server request %d",	s->m_pid, c);
